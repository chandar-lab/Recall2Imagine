...
class GRU_RSSM(RSSM):
  ...
  def __init__(
      self, deter=1024, stoch=32, classes=32, hidden=None, unroll=False, initial='learned',
      unimix=0.01, action_clip=1.0, nonrecurrent_enc=False, **kw):
    self._deter = deter
    self._stoch = stoch
  ...
  def initial(self, bs):
    if self._classes:
      state = dict(
          ...
          hidden=jnp.zeros([bs, self._deter], f32),
  ...
  def _cell(self, x, prev_state):
    ...
    return deter, {'deter': deter, 'hidden': deter}
